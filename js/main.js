...
radioFunction(".team-radio", ".team-block__inner-about", ".team-label", "active-team");
radioFunction(".testim-radio", ".testim-block__rewiev", ".testim-label", "active-team");
radioFunction(".news-radio", ".news-block__news-inner", ".news-label", "active-block");
priceBlocks();
mapFunc();
scrollMenu();
portfolioFunction();
...
function portfolioFunction() {
    ...
    document.querySelectorAll('.portfolio-block__img-over-title a').forEach((e) => {
        e.setAttribute('href', 'https://www.instagram.com/');
    })
    let count = 0;
    document.querySelectorAll('.port-hart img').forEach((e) => {
        e.addEventListener('click', () => {
            e.parentElement.querySelector('img:last-child').classList.toggle('active-lilke');
            if (e.parentElement.querySelector('img:last-child').classList.contains('active-lilke')) {
                count++;
                console.log(count);
    ...
    document.querySelectorAll('.port-chain img').forEach((e) => {
        e.addEventListener('click', () => {
            e.parentElement.querySelector('img:last-child').classList.toggle('active-chain');
        })
    })
}
...
function menuAdaptiveToScroll() {
    window.addEventListener('scroll', () => {
        let autoCheckMenu = function(idMenu, blockToScroll) {
            if (document.querySelector(blockToScroll).getBoundingClientRect().top + 400 < window.innerHeight) {
                document.querySelectorAll('.menu ul li').forEach(e => {
                    e.classList.remove('active-menu');
                });
                document.querySelector(idMenu).classList.add('active-menu');
            } else if (document.querySelector(blockToScroll).getBoundingClientRect().bottom + 400 < window.innerHeight) {
                document.querySelectorAll('.menu ul li').forEach(e => {
                    e.classList.remove('active-menu');
                });
                document.querySelector(idMenu).classList.add('active-menu');
    ...
    number.forEach((number) => {
        ...
        window.addEventListener('scroll', function onScroll() {
            ...
            if (numberTop < window.innerHeight) {
                document.querySelectorAll('.dgrm svg circle:last-child').forEach((e) => {
